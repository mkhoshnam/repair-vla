...
# Generic screened-task launcher for Joint-Factorized Capability RL (JFCRL).
#
# Examples:
#   bash scripts/run_jfcrl_screened_tasks.sh list
#   bash scripts/run_jfcrl_screened_tasks.sh 5 --print-only
#   bash scripts/run_jfcrl_screened_tasks.sh 5 --dry-run
#   bash scripts/run_jfcrl_screened_tasks.sh 5
#   bash scripts/run_jfcrl_screened_tasks.sh 8
#   bash scripts/run_jfcrl_screened_tasks.sh all
#
# A number N means: train on the first N tasks in
# configs/jfcrl_screened_task_pool.tsv.  "all" means all screened tasks.
# The first five entries are the fixed five-task expansion set chosen after the
# successful two-task pilot.  j2 is globally excluded from training.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
TASK_FILE="$REPO_ROOT/configs/jfcrl_screened_task_pool.tsv"

usage() {
  cat <<'USAGE'
Usage:
  bash scripts/run_jfcrl_screened_tasks.sh <N|all|list> [--dry-run|--print-only]
...
Optional environment overrides:
  GPU=1 SEED=7 STEPS_PER_TASK=50000 bash scripts/run_jfcrl_screened_tasks.sh 5
  RUNNAME=my_run bash scripts/run_jfcrl_screened_tasks.sh 5
  SHARED_VLA_CHECKPOINT=<checkpoint> bash scripts/run_jfcrl_screened_tasks.sh 5
USAGE
}
...
RUNNAME=${RUNNAME:-jfcrl_screened_first${N_TASKS}_sharedvla_seed${SEED}}
...
Untouched globally-unseen j2 evaluation on every task stored in the checkpoint:

bash scripts/eval_jfcrl_unseen_j2.sh \
  "$RUNDIR/ckpt_${STEPS}.pt"
EOF2
